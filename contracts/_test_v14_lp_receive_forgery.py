...
Delete that TAIL assert and rebuild (scripts/mutate-v14.py) and B flips to
...
def lp_message(lp_delta: int, new_total_lp: int, next_state_root: bytes32) -> bytes32:
    """The rue mirror: tree_hash(["forge-lp-v14", lp_delta, new_total_lp, next_state_root])."""
    return Program.to(["forge-lp-v14", lp_delta, new_total_lp, next_state_root]).get_tree_hash()
...
def main():
    ...
    # receiver. V12's finalizer accepted that (99.98% of both reserves, chia_rs-verified); V14's
    # asserts the root is the six-leaf root of ONE configuration. The full before-and-after is
    # _test_v12_v14_review_findings.py; this pins the V14 refusal beside the honest control above.
    ...
    # H1/H2 pin `exact_withdrawal`, the multi-line assert scripts/mutate-v14.py cannot
