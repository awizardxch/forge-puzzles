...
import _v11_testkit as kit  # noqa: E402
...
def main() -> int:
    if not kit.v11_available():
        print("  [skip] V11 build outputs are absent; run scripts/build-v11.py"); return 2
    ...
    st1, _, _, eph1 = kit.run_leaf(pool, "forge_action_swap", [H0, 0, 1, g1, gross1, s1.name()])
    st2, _, _, _ = kit.run_leaf(pool, "forge_action_add", [H0, [x_dep, a_dep], minted, bytes32(b"\x01" * 32), [sx.name(), sa.name()]],
    ...
    steps = [("forge_action_swap", [H0, 0, 1, g1, gross1, s1.name()]),
             ("forge_action_add", [H0, [x_dep, a_dep], minted, lp_parent, [sx.name(), sa.name()]]),
             ("forge_action_swap", [H0, 1, 0, a_back, gross3, s3.name()])]
    ...
    if stale != gross3:
        try:
            kit.validate(kit.spend_actions(pool, [steps[0], steps[1], ("forge_action_swap", [H0, 1, 0, a_back, stale, s3.name()])],
    ...
    try:
        kit.validate(kit.spend_actions(pool, [steps[0], ("forge_action_add", [H0, [x_dep, a_dep], minted + 1, lp_parent, [sx.name(), sa.name()]]), steps[2]],
                                       extra_spends=[s1_spend, sx_spend, *lp_spends], extra_cats={CAT: [sa_cat, s3_cat]})[0])
