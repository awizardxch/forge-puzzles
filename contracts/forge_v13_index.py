...
own record in .awizard/v13-testnet.json; this script turns that record into the
index's shape -- one plan per pool, one batch, `poolSnapshot` in the V11 format
forge_v13_offer round-trips -- so the same responder and listing code serves
...
import forge_v13_offer as v13  # noqa: E402

RECORD = ROOT / ".awizard" / "v13-testnet.json"
ZERO_HEX = "00" * 32
...
def pool_from_record(record: dict) -> drv.V13Pool:
    ...
    st = record["state"]
    # V13 records carry [reserves, total_lp, fees_owed, oracle, prev_root, dao_fee_bps, dao_owed, reserve_parents]
    state = [st[0], st[1], st[2], st[3], bytes32.fromhex(st[4]),
             int(st[5]) if len(st) > 5 else 0, list(st[6]) if len(st) > 6 else [0] * len(st[0]),
             [bytes32.fromhex(x) for x in st[7]] if len(st) > 7 else [drv.ZERO_32] * len(st[0])]
    ...
    return replace(pool, coin=_coin(record["coin"]), lineage=_lineage(record["lineage"]),
                   birth=int(record.get("birth") or 0))   # V13
...
def entries(state: dict, resolve: bool = False) -> tuple[dict, list]:
    ...
    for record in state["pools"]:
        try:
            ...
            snapshot = v13.pool_to_snapshot(pool)
            v13.snapshot_to_pool(json.loads(json.dumps(snapshot)))    # proves it, through JSON as node holds it
        ...
        plans[plan_key] = {
            ...
            "batches": {
                "0": {
                    "status": "confirmed",
                    "poolVersion": v13.PROTOCOL_VERSION,
