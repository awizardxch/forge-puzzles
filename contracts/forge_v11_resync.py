...
from forge_v11_offer import pool_to_snapshot, snapshot_to_pool  # noqa: E402
...
def _leaf_names(pool: drv.V11Pool) -> dict[bytes, str]:
...
def replay_spend(pool: drv.V11Pool, solution: Program) -> tuple[drv.V11Pool, list[str]]:
    ...
    for leaf_prog, leaf_sol in zip(puzzles.as_iter(), solutions.as_iter()):
        ...
        new_state, _tagged, _base, ephemeral = drv.run_leaf(
            pool, name, list(leaf_sol.as_iter()), ephemeral=ephemeral, state=new_state)
...
def resync(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"success": True, "changed": bool(steps), "tip_coin_id": tip_id, "steps": steps,
            "snapshot": pool_to_snapshot(pool)}
