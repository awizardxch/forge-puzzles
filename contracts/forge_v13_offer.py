...
class SettleResult:
    bundle: SpendBundle
    pool: drv.V13Pool          # the pool after the spend
...
def pool_to_snapshot(pool: drv.V13Pool) -> dict[str, Any]:
    reserves, total_lp, fees_owed, (last_height, cums, last_spot), prev_root, dao_fee_bps, dao_owed, reserve_parents = pool.state   # V13
    return {
        ...
        "state": {
            ...
            "oracle": {"last_height": int(last_height), "cums": [str(int(c)) for c in cums],
                       "last_spot": [str(int(c)) for c in last_spot]},   # V13
            "prev_root": _hex(prev_root),
            "dao_fee_bps": int(dao_fee_bps),
            "dao_owed": [str(int(x)) for x in dao_owed],
            "reserve_parents": [_hex(p) for p in reserve_parents],   # V13
            "birth": int(pool.birth),   # V13
        },
    }


def snapshot_to_pool(value: dict[str, Any]) -> drv.V13Pool:
    ...
    state = [
        ...
        [int(st["oracle"]["last_height"]), [int(c) for c in st["oracle"]["cums"]],
         [int(c) for c in st["oracle"]["last_spot"]]],   # V13
        _b32(st["prev_root"]),
        int(st.get("dao_fee_bps") or 0),
        [int(x) for x in (st.get("dao_owed") or [0] * len(assets))],
        [_b32(x) for x in st["reserve_parents"]],   # V13: exact, so the re-curried inner hashes to the recorded coin
    ]
    ...
    # V13: the coin's birth height. `pool_to_snapshot` writes this INSIDE `state`, so
    ...
    # offer-lane swap, add and remove failed from the moment V13 went live. Read where it
...
def _finish(offer: Offer, pool: drv.V13Pool, pool_bundle: SpendBundle, new_state, details: dict[str, Any]) -> SettleResult:
...
def _pool_asset_settlements(pool: drv.V13Pool, offer: Offer):
...
def settle_swap(pool: drv.V13Pool, offer: Offer, height: int, surplus_ph: bytes32,
...
def _mint(pool: drv.V13Pool, deposits: list[int]) -> int:
    return forge_math.invariant_lp_mint(pool.state[0], deposits, pool.state[1], pool.fee_bps, pool.weights, version=MATH_VERSION)


def settle_add(pool: drv.V13Pool, offer: Offer, height: int, surplus_ph: bytes32) -> SettleResult:
    offered, wanted = _pool_asset_settlements(pool, offer)
...
def settle_remove(pool: drv.V13Pool, offer: Offer, height: int, surplus_ph: bytes32) -> SettleResult:
...
def settle(action: str, pool: drv.V13Pool, offer: Offer, height: int, surplus_ph: bytes32 | None = None,
           fee_bps: int = 0) -> SettleResult:
