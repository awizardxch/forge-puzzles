...
pools and 12 launch-matrix steps ran on has to arrive in V12 unchanged, or the
...
port to that:
  ...
  1. THE CORPUS. compiled/v10-spend-corpus.json holds every V10 pool spend that
     ...
     to the compiled V12 functions, which must accept exactly the recorded
  ...
  2. THE V10 FUNCTIONS THEMSELVES. scripts/build-v12.py compiles the curve
...
COMPILED = CONTRACTS / "v12" / "compiled"
...
class Suite:
    def __init__(self):
        self.cur = {fn: load(f"forge_curve.{fn}") for fn in (
    ...
    def same(self, fn, cur_args, v10_args, label):
        a, b = run(self.cur[fn], *cur_args), run(self.v10[fn], *v10_args)
        self.expect(f"{label}: V12 {a} vs V10 {b}", a == b)
        return a
                    ...
                    for i, plan in enumerate(act["reserve_plans"]):
                        ...
                        if i == dn:
                            self.expect(f"{tag} protocol_fee_owed",
                                        run(self.cur["protocol_fee_owed"], released, pbps) == ("ok", owed))
                            self.expect(f"{tag} valid_protocol_fees accepts",
                                        run(self.cur["valid_protocol_fees"], owed, released, pbps) == ("ok", 1))
                            self.expect(f"{tag} valid_protocol_fees refuses +1",
                                        run(self.cur["valid_protocol_fees"], owed + 1, released, pbps) == ("ok", 0))
                        else:
...
def main() -> int:
    ...
    if not (COMPILED / "forge_curve.exact_swap_output.rue.hex").is_file():
        print("  [skip] V12 build outputs are absent; run scripts/build-v12.py")
        return 2
    ...
    print(f"randomized: {cases} pools driven through both the V10 and the V12 functions")
    print()
