#!/usr/bin/env python3
"""V13 genesis binds to exactly one LP eve (CHIP-0062 review P0).
...
V13 puts the eve's own coin id into the announced list: the launcher announces
...
def main():
    ...
    if out:
        ...
        # V13: the settlement pays the locked floor to the zero puzzle hash and the rest to the creator
        check("  the creator received exactly total_lp - MIN_LOCKED_LP", sum(a for ph, a in additions if ph == lp_ph) == total - kit.MIN_LOCKED_LP)
        check("  and MIN_LOCKED_LP was burned to the zero puzzle hash", sum(a for ph, a in additions if ph == burn_ph) == kit.MIN_LOCKED_LP)
    ...
    check("register_solution carries the eve state's reserve parents", sol[5] == pool.state[7])

    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V13 genesis checks")
