...
from forge_v13_offer import OfferRejected, pool_to_snapshot  # noqa: E402
...
def plan(registry: drv.Registry, slots: dict, config: CreationConfig, xch: CreatorXch, cats: dict,
    ...
    if config.total_lp <= drv.MIN_LOCKED_LP:
        raise OfferRejected(f"the genesis supply must exceed the locked minimum of {drv.MIN_LOCKED_LP} LP units")
    ...
    # V13: the creator receives everything above the locked floor; the floor is burned by the same
    ...
    conditions.append([ASSERT_PUZZLE_ANNOUNCEMENT, lp_payment_announcement(pool.lp_asset_id, launcher_id, recipient_ph,
                                                                          config.total_lp - drv.MIN_LOCKED_LP)])
    ...
    pool.extra["eve_coin_id"] = eve.name()   # V13: register_solution reads it
    launcher_spend = make_spend(launcher, SINGLETON_LAUNCHER, Program.to([pool.coin.puzzle_hash, 1, [config.total_lp, eve.name()]]))  # V13: names the one eve
    eve_spends = drv.lp_eve_ring(pool, eve, OFFER_PH, config.total_lp, drv.genesis_action(pool))
    lp_pay = drv.genesis_lp_settlement_spends(pool, eve, recipient_ph, config.total_lp)   # V13: burns the floor
    ...
    return CreationPlan(pool, registry_after, key, left, right, recipient_ph, creator_spends, router_spends, network_fee, {
        ...
        "need_xch": need, "change": change, "genesis_lp": config.total_lp, "burned_lp": drv.MIN_LOCKED_LP,
        "creator_lp": config.total_lp - drv.MIN_LOCKED_LP, "name": config.name, "symbol": config.symbol,
    })
    ...
    return {
        "pool": {
            ...
            "state": [st[0], st[1], st[2], st[3], bytes(st[4]).hex(), int(st[5]), list(st[6]),
                      [bytes(x).hex() for x in st[7]]],   # V13: reserve parents live in state
