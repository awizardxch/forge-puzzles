#!/usr/bin/env python3
"""The Forge V14 Python driver: every puzzle construction and spend assembly.
...
What lives here:
  * loading the compiled V14 puzzles from contracts/v14/compiled;
  * V14: the reserve launcher -- every reserve is born from forge_reserve_launcher,
    whose parent (the grandparent) is what `register` derives the reserve parent
    from; `reserve_launcher_spends` builds the launchers' spends for a genesis, and
    every leaf names its settlement by (parent, amount) -- `settlement_ref`;
  * PoolSpec-free construction of a V14 pool (V14Pool): reserve inners on
...
# broken build without ever writing into contracts/v14/compiled -- leaving a
# mutant behind in the real build is the one way this tooling could do harm.
V14_COMPILED = (pathlib.Path(os.environ["FORGE_V14_COMPILED"]).resolve()
                if os.environ.get("FORGE_V14_COMPILED")
                else CONTRACTS / "v14" / "compiled")
...
RESERVE_TAG = -42
PROTOCOL_VERSION = 15      # V14 (fourth-review revision); V13 was 14, V11.1 was 12
LOCKED_BURN = 1            # V14: mirrors forge_action_common.rue -- the genesis burn, the register floor, the remove cap
PRICE_SCALE = 2 ** 64
ORACLE_WINDOW = 32
...
def v14_program(name: str) -> Program:
    path = V14_COMPILED / f"{name}.rue.hex"
    if not path.is_file():
        path = V14_COMPILED / f"{name}.clvm.hex"
    return Program.from_bytes(bytes.fromhex(path.read_text().strip()))


def v14_available() -> bool:
    return (V14_COMPILED / "forge_action_swap.rue.hex").is_file()
...
if v14_available():
    ACTION_LAYER = v14_program("action")
    P2_DELEGATED = v14_program("p2_delegated_by_singleton")
    SLOT = v14_program("slot")
    FINALIZER_MOD = v14_program("forge_multi_reserve_finalizer")
    RESERVE_AMOUNT = v14_program("forge_reserve_amount")
    PASSTHROUGH = v14_program("passthrough_action")
    PASSTHROUGH_OBSERVE = v14_program("passthrough_observe")
    LP_TAIL_MOD = v14_program("forge_lp_cat_tail")
    LP_MINT_INNER = v14_program("forge_lp_mint_inner")
    LP_MELT_INNER = v14_program("forge_lp_melt_inner")
    LEAF_MODS = {name: v14_program(name) for name in LEAF_ORDER}
    RESERVE_LAUNCHER = v14_program("forge_reserve_launcher")       # V14
    ...
    LEAF_MODS = {}
    RESERVE_LAUNCHER = None
RESERVE_LAUNCHER_HASH = RESERVE_LAUNCHER.get_tree_hash() if RESERVE_LAUNCHER is not None else None


# ---- V14: the reserve launcher and the settlement reference ---------------------------------

def reserve_launcher_message(created_puzzle_hash: bytes, amount: int, launcher_id: bytes) -> bytes:
    """What forge_reserve_launcher announces (a COIN announcement) about the coin it created.
    The ASCII prefix keeps it from starting 0xcb, which the CAT layer refuses from an inner."""
    return b"forge-reserve-v14" + Program.to([bytes32(created_puzzle_hash), amount, bytes32(launcher_id)]).get_tree_hash()


def reserve_launcher_full_hash(asset_id) -> bytes32:
    """The launcher coin's puzzle hash: bare for XCH, CAT-wrapped for a CAT."""
    if asset_id is None:
        return bytes32(RESERVE_LAUNCHER_HASH)
    return bytes32(construct_cat_puzzle(CAT_MOD, asset_id, RESERVE_LAUNCHER).get_tree_hash())


def settlement_ref(coin: Coin) -> tuple[bytes32, int]:
    """V14: a leaf names its settlement by parent and amount and derives the id itself
    (spec 1.8). Every swap and add solution takes `*settlement_ref(coin)` where V13 took
    `coin.name()`."""
    return bytes32(coin.parent_coin_info), int(coin.amount)


def settlement_refs(coins) -> tuple[list, list]:
    """The add leaf's two lists, (parents, amounts), one entry per coin in asset order."""
    return [bytes32(c.parent_coin_info) for c in coins], [int(c.amount) for c in coins]
...
def expected_oracle(state, weights, h: int, birth: int | None = None, scale: int = PRICE_SCALE) -> list:
    ...
    what make_pool gives a fabricated pool (see V14Pool.birth)."""
...
class Reserve:
    ...
    # V14: the launcher's parent (what `register` is given, and derives the reserve parent
    # from) and, for a CAT, the launcher coin's own lineage proof. Genesis only; a successor
    # reserve keeps them but nothing reads them after registration.
    grandparent: bytes32 | None = None
    launcher_lineage: LineageProof | None = None
...
class V14Pool:
    ...
    def advance(self, new_state) -> "V14Pool":
...
def make_pool(asset_ids: list, reserves: list[int], total_lp: int = 1_000_000,
              ...
              dao_ph: bytes32 | None = None, dao_fee_bps: int = 0) -> V14Pool:
    ...
    for i, (asset, amount) in enumerate(zip(asset_ids, amounts)):
        ...
        grandparent, launcher_lineage = None, None
        if reserve_coins is not None:
            # (coin, lineage) for a live reserve; (coin, lineage, grandparent[, launcher_lineage])
            # for a genesis reserve the registration bundle must launch.
            entry = tuple(reserve_coins[i])
            coin, lineage = entry[0], entry[1]
            if len(entry) > 2:
                grandparent = entry[2]
                launcher_lineage = entry[3] if len(entry) > 3 else None
            ...
            # V14: a reserve is the child of a reserve launcher. The launcher's parent -- the
            # grandparent -- is what `register` is given; the parent is derived from it.
            full_hash = inner_hash
            grandparent = bytes32(bytes([0x70 + i, salt]) * 16)
            parent = coin_id(grandparent, RESERVE_LAUNCHER_HASH, amount)
            coin = Coin(parent, full_hash, uint64(amount))
            lineage = None
        else:
            full_hash = construct_cat_puzzle(CAT_MOD, asset, inner).get_tree_hash()
            # The grandparent is a CAT coin of this asset (the creator's), itself fabricated from a
            # great-grandparent so the launcher's lineage proof is one the CAT layer accepts.
            great_grandparent = bytes32(bytes([0x90 + i, salt]) * 16)
            grandparent = coin_id(great_grandparent, construct_cat_puzzle(CAT_MOD, asset, IDENTITY).get_tree_hash(), amount)
            launcher_lineage = LineageProof(great_grandparent, IDENTITY.get_tree_hash(), uint64(amount))
            parent = coin_id(grandparent, reserve_launcher_full_hash(asset), amount)
            coin = Coin(parent, full_hash, uint64(amount))
            lineage = LineageProof(grandparent, RESERVE_LAUNCHER_HASH, uint64(amount))
        built.append(Reserve(i, asset, inner, bytes32(full_hash), coin, lineage, grandparent, launcher_lineage))
    ...
    pool = V14Pool(launcher_parent, launcher_id, struct_hash, launcher_id, list(asset_ids), list(weights),
...
def run_leaf(pool: V14Pool, name: str, solution: list, program: Program | None = None,
...
def delegated_puzzle_for(pool: V14Pool, reserve: Reserve, new_state, tagged_in_emission_order: list) -> Program:
...
def assemble(pool: V14Pool, leaf: Program, proof: list, solution: list, new_state, tagged_conditions: list,
...
def spend_pool(pool: V14Pool, new_state, tagged_conditions=(), base_conditions=(), **knobs) -> SpendBundle:
...
def spend_action(pool: V14Pool, name: str, solution: list, leaf: Program | None = None,
...
def lp_mint_spends(pool: V14Pool, lp_delta: int, new_total_lp: int, next_state_root: bytes32,
...
def lp_eve_ring(pool: V14Pool, eve: Coin, recipient_ph: bytes32, mint: int, action: list):
...
def genesis_action(pool: V14Pool) -> list:
...
def lp_melt_spend(pool: V14Pool, burn: int, new_total_lp: int, next_state_root: bytes32,
...
def registry_available() -> bool:
    return (V14_COMPILED / "forge_registry_register.rue.hex").is_file()


if registry_available():
    DEFAULT_FINALIZER = v14_program("finalizer")
    REGISTRY_LEAF_MODS = {name: v14_program(name) for name in REGISTRY_LEAF_ORDER}
else:
...
class Registry:
    ...
    @property
    def reserve_launcher_hash(self) -> bytes32:
        return bytes32(self.constants[14])
...
def make_registry(salt: int = 0x21, creation_fee: int = 1_000_000, treasury_ph: bytes32 | None = None,
    ...
    constants = [
        ...
        RESERVE_LAUNCHER_HASH,                            # V14: the only puzzle a reserve is born from
    ]
...
def launcher_spend(pool: V14Pool, kv_list=None):
...
def reserve_launcher_spends(pool: V14Pool, indices=None, launcher_id: bytes32 | None = None,
                            amounts: dict | None = None, created: dict | None = None) -> list:
    """V14: the reserve launchers' spends, one per reserve: the coin at the launcher puzzle (CAT-
    wrapped for a CAT) parented by the grandparent `register` is given, creating the reserve at
    its inner hash for its whole amount, hinted and announced. Every registration bundle carries
    these; `register` asserts each announcement from the parent it derives.

    The knobs exist for the suites: `launcher_id`, `amounts` (index -> amount) and `created`
    (index -> puzzle hash) let a probe make a launcher announce or create the wrong thing."""
    spends, cats = [], {}
    for r in pool.reserves:
        if indices is not None and r.index not in indices:
            continue
        assert r.grandparent is not None, f"reserve {r.index} has no grandparent: a genesis is built through the launcher"
        amount = int(r.coin.amount)
        launcher = Coin(r.grandparent, reserve_launcher_full_hash(r.asset_id), uint64(amount))
        assert launcher.name() == r.coin.parent_coin_info, "the reserve's parent is the launcher derived from its grandparent"
        solution = Program.to([(created or {}).get(r.index, r.inner_hash), (amounts or {}).get(r.index, amount),
                               launcher_id if launcher_id is not None else pool.launcher_id])
        if r.asset_id is None:
            spends.append(make_spend(launcher, RESERVE_LAUNCHER, solution))
        else:
            cats.setdefault(r.asset_id, []).append(SpendableCAT(
                launcher, r.asset_id, RESERVE_LAUNCHER, solution, lineage_proof=r.launcher_lineage))
    for asset, spendables in cats.items():
        try:
            spends.extend(unsigned_spend_bundle_for_spendable_cats(CAT_MOD, spendables).coin_spends)
        except ValueError as exc:
            raise Rejected(f"reserve launcher ring: {exc}") from exc
    return spends
...
def genesis_lp_settlement_spends(pool: V14Pool, eve: Coin, recipient_ph: bytes32, mint: int,
    ...
    `burn` defaults to LOCKED_BURN; a test may name another amount to prove the assert."""
    burn = LOCKED_BURN if burn is None else burn
...
def lp_genesis_mint_spends(pool: V14Pool, recipient_ph: bytes32, salt: int = 0xE4, mint: int | None = None,
...
def register_solution(pool: V14Pool, left: tuple, right: tuple, grandparents: list | None = None) -> list:
    """`left` / `right` are (key, launcher_id, far_key) of the two adjacent slots.
    V14: the solution carries each reserve's GRANDPARENT; `register` derives the parent from it
    and the launcher's hash, so the parents in the eve state are never claimed (spec 1.4)."""
    if grandparents is None:
        assert all(r.grandparent is not None for r in pool.reserves), "a genesis is built through the launcher"
        grandparents = [r.grandparent for r in pool.reserves]
    return [pool.launcher_parent, pool.config(), pool.state[0], pool.state[1], pool.state[5],
            grandparents, pool.extra.get("eve_coin_id", genesis_eve_id(pool)),
