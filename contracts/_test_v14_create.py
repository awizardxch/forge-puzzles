...
CAT asset. `forge_v14_create.plan` splits the creation bundle into the creator's
...
def main() -> int:
    ...
    # V14: the settlement burns the locked floor and pays the creator the rest
    ...
    check("  and LOCKED_BURN is burned to the zero puzzle hash", paid_to(bundle, drv.ZERO_32, p.pool.lp_asset_id) == drv.LOCKED_BURN)
    ...
    snap = v14.pool_to_snapshot(p.pool)
    check("  the successor pool snapshot rebuilds", v14.snapshot_to_pool(snap).coin == p.pool.coin)
    ...
    for cs in p.router_spends:
        ...
        if bytes(CREATOR_PH) in sol and cs.coin.puzzle_hash == construct_cat_puzzle(CAT_MOD, p.pool.lp_asset_id, v14.OFFER_MOD).get_tree_hash():
    ...
    for label, fn in (
        ...
        except v14.OfferRejected as exc:
