#!/usr/bin/env python3
"""Every message V13 sends or receives, and what binds each end of it.
...
`_test_v13_lp_receive_forgery.py`; the reserve messages are
`_test_v13_finalizer.py`; the DAO message is `_test_v13_dao_fee.py`; the registry
and genesis announcements are `_test_v13_registry.py` and `_test_v13_genesis.py`.
...
def lp_message(delta, new_total, root):
    return Program.to(["forge-lp-v13", delta, new_total, root]).get_tree_hash()
...
def main() -> int:
    if not kit.v13_available():
        print("  [skip] the V13 build is absent; run scripts/build-v13.py")
        return 2

    pool = kit.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                         leaves="forge", salt=0x90, last_height=H - 40)
    pool = kit.replace(pool, birth=H - 39)   # V13: birth > last_height
    ...
    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V13 message-binding checks")
