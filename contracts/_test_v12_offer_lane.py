...
import forge_stdin
import forge_v12_driver as drv
import forge_v12_offer as v12
...
def snapshot_roundtrip(pool):
    snap = v12.pool_to_snapshot(pool)
    back = v12.snapshot_to_pool(json.loads(json.dumps(snap)))
    return snap, back
...
def swap_case(label, pool, i_in, i_out, gross, ask_bps=9_900, expect_ok=True):
    ...
    honest = v12.forge_math.swap_output(r[i_in], r[i_out], gross, pool.fee_bps, w[i_in], w[i_out])
    ...
    check("  successor snapshot is V11 and rebuilds to the successor pool",
          out["pool"]["protocol_version"] == drv.PROTOCOL_VERSION and v12.snapshot_to_pool(out["pool"]).coin.name().hex() == out["forge"]["successor_coin_id"])
...
def add_case(label, pool, pct, extra_xch=0, expect_ok=True):
    ...
    honest = v12.forge_math.invariant_lp_mint(pool.state[0], deposits, pool.state[1], pool.fee_bps, pool.weights, version=10)
    offered = {}
    ...
    check("  successor snapshot rebuilds", v12.snapshot_to_pool(out["pool"]).coin.name().hex() == out["forge"]["successor_coin_id"])
    return out
...
def remove_case(label, pool, burn, ask_bps=9_900, expect_ok=True):
    ...
    snap, _ = snapshot_roundtrip(pool)
    vf = v12.forge_math.vault_fee_bps(len(pool.state[0]), 10, pool.fee_bps)
    payouts = v12.forge_math.withdrawal_amounts(pool.state[0], burn, pool.state[1], vf)
...
def main() -> int:
    ...
    bad = dict(v12.pool_to_snapshot(pair)); bad["state"] = dict(bad["state"]); bad["state"]["total_lp"] = "999"
    try:
        v12.snapshot_to_pool(bad); check("  a snapshot with altered state is refused", False)
    except ValueError as exc:
        check("  a snapshot with altered state is refused", "does not hash" in str(exc))
    try:
        forge_stdin._pool(v12.pool_to_snapshot(pair)); check("  the V3Pool reader refuses a V11 snapshot", False)
    except ValueError as exc:
        check("  the V3Pool reader refuses a V12 snapshot", "versioned offer lane" in str(exc))
    ...
    print("lane guards:")
    snap = v12.pool_to_snapshot(pair)
    ...
    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} V12 offer-lane checks passed")
