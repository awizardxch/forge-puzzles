#!/usr/bin/env python3
"""The V12 build is what it claims to be: upstream pinned, curve fresh.

Two kinds of drift this catches, and both would be silent otherwise:

  * an upstream puzzle that is not the community-reviewed one. V12 leans on
...
def main() -> int:
    if not (COMPILED / "manifest.json").is_file():
        print("  [skip] contracts/v12/compiled/manifest.json is absent; run scripts/build-v12.py")
        return 2
    ...
    common = (V12 / "puzzles" / "forge_action_common.rue").read_text(encoding="utf-8")
    ...
    if shutil.which("rue"):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            scratch = pathlib.Path(tmp) / "v12"
            shutil.copytree(V12, scratch, ignore=shutil.ignore_patterns("compiled"))
    ...
    strays = sorted(p.name for p in V12.rglob("*_v[0-9]*.rue"))
    results.append(check("no versioned puzzle names in the V12 set", not strays, ", ".join(strays)))

    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} V12 integrity checks passed")
