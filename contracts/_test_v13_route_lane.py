...
Fabricated wallet offers (see _test_v13_offer_lane) drive multi-hop, split,
...
import forge_v13_route as v13r  # noqa: E402
from _test_v13_offer_lane import ROUTER_PH, T_A, T_B, TRADER_PH, H, fabricate_offer, make, paid_to
...
def snap(pool):
    return v13.pool_to_snapshot(pool)
...
def attempt(label, payload, expect_ok=True):
    ...
    if not check(f"{label}: builder accepted", expect_ok):
        return None
    for s in out["pools"]:
        check("  successor snapshot rebuilds", v13.snapshot_to_pool(s).coin.puzzle_hash.hex() == s["pool_coin"]["puzzle_hash"])
    return out
...
def main() -> int:
    ...
    gross_w, backing = v13r.wrap_backing([xa, vault, xlp], [hx(None), hx(T_A), hx(vault.lp_asset_id), hx(None)], offered_x)
    a_out, lp_mint, x_back = v13r.simulate_chain([xa, vault, xlp], [hx(None), hx(T_A), hx(vault.lp_asset_id), hx(None)], gross_w)
    ...
    dep_a = 50_000
    lp_mint2, x_back2 = v13r.simulate_chain([vault, xlp], [hx(T_A), hx(vault.lp_asset_id), hx(None)], dep_a)
    ...
    gross_r, backing_r = v13r.wrap_backing(rpools, rpath, offered_r)
    outs_r = v13r.simulate_chain(rpools, rpath, gross_r)
    ...
    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} V13 route-lane checks passed")
