...
deployment record (.awizard/v14-testnet.json) and asks the node the questions a
...
RECORD = pathlib.Path(__file__).resolve().parent.parent / ".awizard" / "v14-testnet.json"
...
def main() -> int:
    if not RECORD.is_file():
        print("  [skip] no deployment record; run scripts/deploy-v14-testnet.py first")
        return 2
    ...
    for pool in state["pools"]:
        ...
        # A pool created after the genesis burn landed sends LOCKED_BURN to the burn
