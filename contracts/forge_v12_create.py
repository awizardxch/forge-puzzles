...
from forge_v12_offer import OfferRejected, pool_to_snapshot  # noqa: E402
...
def plan(registry: drv.Registry, slots: dict, config: CreationConfig, xch: CreatorXch, cats: dict,
    ...
    pool.extra["eve_coin_id"] = eve.name()   # V12: register_solution reads it
    launcher_spend = make_spend(launcher, SINGLETON_LAUNCHER, Program.to([pool.coin.puzzle_hash, 1, [config.total_lp, eve.name()]]))  # V12: names the one eve
    ...
    return {
        "pool": {
            ...
            "state": [st[0], st[1], st[2], st[3], bytes(st[4]).hex(), int(st[5]), list(st[6]),
                      [bytes(x).hex() for x in st[7]]],   # V12: reserve parents live in state
