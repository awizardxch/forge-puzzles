...
def main() -> int:
    ...
    if out:
        ...
        check("  oracle accumulated the pre-spend price", st[3] == [H0, kit.expected_cums(pool.state, pool.weights, H0)])
    ...
    # V12 (CHIP-0062 review P1, "the final LP position cannot redeem"): everything above the
    # locked minimum is redeemable, and exactly the minimum stays.
    accepts("V12: burning everything above MIN_LOCKED_LP (total_lp - 1000) is accepted",
            lambda: kit.validate(remove_case(three, 3_000_000 - kit.MIN_LOCKED_LP, salt=0x95)[0]))
    refuses("V12: one unit into the locked minimum is refused",
    ...
    if out:
        ...
        st = kit.state_to_list(new_state)
        check("  state unchanged beyond the prologue", st[0] == pool.state[0] and st[1] == pool.state[1] and st[3] == [H0, cums])
