...
def swap_case(pool, asset_in, asset_out, gross, h=H0, claimed=None, settle=True, salt=0xC0):
    ...
    extra_spends, extra_cats, settlement = [], {}, (bytes32(b"\x00" * 32), gross)
    if settle:
        if asset is None:
        ...
        settlement = kit.settlement_ref(coin)
    bundle, new_state = kit.spend_action(pool, "forge_action_swap",
                                         [h, asset_in, asset_out, gross, claimed, *settlement],
...
def main() -> int:
    ...
    refuses("a negative swap input is refused",
            leaf_probe("forge_action_swap", [H0, 0, 1, -250_000, 100_000, SETTLE_ID, 250_000]))
    refuses("a zero swap input is refused",
            leaf_probe("forge_action_swap", [H0, 0, 1, 0, 100_000, SETTLE_ID, 250_000]))
    refuses("a negative claimed output is refused",
            leaf_probe("forge_action_swap", [H0, 0, 1, 250_000, -100_000, SETTLE_ID, 250_000]))
    refuses("a zero claimed output is refused",
            leaf_probe("forge_action_swap", [H0, 0, 1, 250_000, 0, SETTLE_ID, 250_000]))
    refuses("a negative asset index is refused",
            leaf_probe("forge_action_swap", [H0, -1, 1, 250_000, 100_000, SETTLE_ID, 250_000]))
    refuses("an asset index past the last reserve is refused",
            leaf_probe("forge_action_swap", [H0, 0, 9, 250_000, 100_000, SETTLE_ID, 250_000]))
    # V14 (mutation rule): the remove cases above size their payouts through the mirror, whose
    # own guard raises for a burn past the floor before the leaf runs -- so the leaf's
    # `burn <= total_lp - LOCKED_BURN` was reported UNREACHED. Size the payouts by hand.
    refuses("burning the WHOLE supply with payouts sized by hand (past LOCKED_BURN) is refused by the leaf",
            leaf_probe("forge_action_remove", [H0, pool.state[1], bytes32(b"\x01" * 32), list(pool.state[0])]))
    refuses("a negative payout slot is refused",
            leaf_probe("forge_action_remove", [H0, 1_000, bytes32(b"\x01" * 32), [-1, 4_000]]))
    ...
    ADD_AMOUNTS = [500_000] * 2
    refuses("a negative LP mint is refused",
            leaf_probe("forge_action_add", [H0, [100_000, 200_000], -50_000, ADD_PARENT, ADD_IDS, ADD_AMOUNTS]))
    refuses("a zero LP mint is refused",
            leaf_probe("forge_action_add", [H0, [100_000, 200_000], 0, ADD_PARENT, ADD_IDS, ADD_AMOUNTS]))
    ...
    refuses("PR-217 I-1: a negative deposit with an invariant-valid positive mint is refused",
            leaf_probe("forge_action_add", [H0, [-100_000, 500_000], 36_611, ADD_PARENT, ADD_IDS, ADD_AMOUNTS]))
    refuses("a deposit of nothing at all is refused",
            leaf_probe("forge_action_add", [H0, [0, 0], 50_000, ADD_PARENT, ADD_IDS, ADD_AMOUNTS]))
    ...
    def add_case(pool, deposits, lp_delta=None, h=H0, settle=(True, True), eve_delta=None, salt=0xA0):
        ...
        probe_state, _, _, _ = kit.run_leaf(pool, "forge_action_add", [h, deposits, lp_delta, bytes32(b"\x01" * 32),
                                                                        [bytes32(b"\x02" * 32)] * len(deposits), list(deposits)])
        ...
        extra_spends, extra_cats, parents, amounts = list(lp_spends), {}, [], []
        for i, (asset, dep, ok) in enumerate(zip(pool.asset_ids, deposits, settle)):
            if dep > 0 and ok:
                if asset is None:
                    ...
                    extra_cats[asset] = [spendable]
                parents.append(coin.parent_coin_info); amounts.append(int(coin.amount))
            else:
                parents.append(bytes32(b"\x00" * 32) if not ok else bytes32(b"\x03" * 32)); amounts.append(dep)
        bundle, new_state = kit.spend_action(pool, "forge_action_add", [h, deposits, lp_delta, lp_parent, parents, amounts],
                                             extra_spends=extra_spends, extra_cats=extra_cats)
    ...
    # V14 (CHIP-0062 review P1, "the final LP position cannot redeem"): everything above the
    # locked minimum is redeemable, and exactly the minimum stays.
    accepts("V14: burning everything above LOCKED_BURN (total_lp - 1) is accepted",
            lambda: kit.validate(remove_case(three, 3_000_000 - kit.LOCKED_BURN, salt=0x95)[0]))
    refuses("V14: one unit into the locked minimum is refused",
            lambda: kit.validate(remove_case(three, 3_000_000 - kit.LOCKED_BURN + 1, salt=0x96)[0]))
    ...
    # V14 (second audit M-1): protocol and DAO fees to the SAME recipient are one coin, not two
    ...
    # V14 (second audit, LOCKED_BURN boundary): the registry refuses a genesis at or below the
    ...
    starved = kit.make_pool([None, CATS[0]], [10_000_000, 20_000_000], total_lp=kit.LOCKED_BURN - 1,
                            leaves="forge", salt=0x3E)
    refuses("a pool minted below LOCKED_BURN cannot be spent at all",
            lambda: kit.validate(kit.spend_action(starved, "forge_action_observe", [H0])[0]))
    floor_exact = kit.make_pool([None, CATS[0]], [10_000_000, 20_000_000], total_lp=kit.LOCKED_BURN,
    ...
    bundle, st = kit.spend_actions(pool, [("forge_action_swap", [H0, 0, 1, g1, out1, *kit.settlement_ref(s1)]),
    ...
    refuses("sandwich: the second swap cannot claim its pre-spend quote",
            lambda: kit.validate(kit.spend_actions(pool, [("forge_action_swap", [H0, 0, 1, g1, out1, *kit.settlement_ref(s1)]),
                                                          ("forge_action_swap", [H0, 0, 1, g2, stale, *kit.settlement_ref(s2)])],
