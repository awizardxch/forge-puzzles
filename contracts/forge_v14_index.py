...
own record in .awizard/v14-testnet.json; this script turns that record into the
index's shape -- one plan per pool, one batch, `poolSnapshot` in the V11 format
forge_v14_offer round-trips -- so the same responder and listing code serves
...
def pool_from_record(record: dict) -> drv.V14Pool:
    ...
    st = record["state"]
    # V14 records carry [reserves, total_lp, fees_owed, oracle, prev_root, dao_fee_bps, dao_owed, reserve_parents]
    ...
    return replace(pool, coin=_coin(record["coin"]), lineage=_lineage(record["lineage"]),
                   birth=int(record.get("birth") or 0))   # V14
...
def entries(state: dict, resolve: bool = False) -> tuple[dict, list]:
    ...
    for record in state["pools"]:
        try:
            ...
            snapshot = v14.pool_to_snapshot(pool)
            v14.snapshot_to_pool(json.loads(json.dumps(snapshot)))    # proves it, through JSON as node holds it
        ...
        plans[plan_key] = {
            ...
            "batches": {
                "0": {
                    "status": "confirmed",
                    "poolVersion": v14.PROTOCOL_VERSION,
