...
CAT asset. `forge_v13_create.plan` splits the creation bundle into the creator's
...
import forge_v13_create as create
...
def main() -> int:
    if not (drv.v13_available() and drv.registry_available()):
        print("  [skip] V13 build outputs are absent"); return 2
    ...
    # V13: the settlement burns the locked floor and pays the creator the rest
    check("  the genesis LP lands at the creator, exactly total_lp - MIN_LOCKED_LP", paid_to(bundle, CREATOR_PH, p.pool.lp_asset_id) == 50_000 - drv.MIN_LOCKED_LP)
    check("  and MIN_LOCKED_LP is burned to the zero puzzle hash", paid_to(bundle, drv.ZERO_32, p.pool.lp_asset_id) == drv.MIN_LOCKED_LP)
    check("  the plan reports both", p.details["burned_lp"] == drv.MIN_LOCKED_LP and p.details["creator_lp"] == 50_000 - drv.MIN_LOCKED_LP)
    ...
    snap = v13.pool_to_snapshot(p.pool)
    check("  the successor pool snapshot rebuilds", v13.snapshot_to_pool(snap).coin == p.pool.coin)
    ...
    for cs in p.router_spends:
        ...
        if bytes(CREATOR_PH) in sol and cs.coin.puzzle_hash == construct_cat_puzzle(CAT_MOD, p.pool.lp_asset_id, v13.OFFER_MOD).get_tree_hash():
    ...
    for label, fn in (
        ...
        except v13.OfferRejected as exc:
    ...
    check("  its genesis LP lands at the creator, less the burned floor", paid_to(bundle2, CREATOR_PH, p2.pool.lp_asset_id) == 30_000 - drv.MIN_LOCKED_LP)
    try:
    ...
    check("  mints 100,000 LP against 10,000 of the asset, 99,000 to the creator", paid_to(bundle3, CREATOR_PH, p3.pool.lp_asset_id) == 100_000 - drv.MIN_LOCKED_LP)
