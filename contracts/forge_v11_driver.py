...
from forge_v11_merkle import LEAF_ORDER, MerkleTree
...
def expected_cums(state, weights, h: int, scale: int = PRICE_SCALE) -> list[int]:
    """The oracle accumulator the prologue must produce for a spend at height h."""
    r, (last_h, cums) = state[0], state[3]
    elapsed = h - last_h
...
def make_pool(asset_ids: list, reserves: list[int], total_lp: int = 1_000_000,
    ...
    protocol_ph = protocol_ph if protocol_ph is not None else bytes32(b"\x7e" * 32)
    ...
    if leaves is None:
        curried = {"passthrough_action": PASSTHROUGH}
        order = ["passthrough_action"]
    elif leaves == "forge":
...
def make_registry(salt: int = 0x21, creation_fee: int = 1_000_000, treasury_ph: bytes32 | None = None,
                  launcher_parent: bytes32 | None = None, state: list | None = None) -> Registry:
...
def lp_genesis_mint_spends(pool: V11Pool, recipient_ph: bytes32, salt: int = 0xE4, mint: int | None = None,
    ...
    rather than by a pool message. Returns [coin_spends]."""
