...
def registration(reg, pool, left, right, slots, fee=None, kv=None, launcher_pool=None, with_fee=True,
                 with_launcher=True, genesis_mint=None, genesis_cat_parent=False, burn=None):
...
def main() -> int:
    ...
    print("register:")
    # V13 (CHIP-0062 review P1, "the final LP position cannot redeem"): the locked minimum is
    # set at genesis, so a supply below it can never be registered.
    low = kit.make_pool([None, CATS[0]], [10_000_000, 20_000_000], total_lp=kit.MIN_LOCKED_LP - 1, leaves="forge", salt=0x5F)
    refuses("V13: a genesis supply below MIN_LOCKED_LP is refused at registration",
    ...
    if out:
        ...
        # V13: the settlement burns the floor and pays the creator the rest; `register` asserted the burn
        check("  the genesis LP above the locked floor landed at the creator, hinted, authorized by the launcher",
              any(p == lp_ph and a == pool_a.state[1] - kit.MIN_LOCKED_LP and h == bytes(RECIPIENT) for p, a, h in hinted))
        check("  and MIN_LOCKED_LP of it was burned to the zero puzzle hash in the same bundle",
              any(p == burn_ph and a == kit.MIN_LOCKED_LP for p, a, _ in hinted))
    ...
    # ---- V13: what the second audit found free ----
    print("V13 admission (second audit):")
    ...
    refuses("a genesis that burns one unit short of the floor is refused",
            lambda: kit.validate(registration(reg2, pool_b, left_b, right_b, slots, burn=kit.MIN_LOCKED_LP - 1)[0]))
    boundary = kit.make_pool([None, CATS[4]], [1_000_000, 1_000_000], total_lp=kit.MIN_LOCKED_LP, leaves="forge", salt=0x56)
    refuses("a genesis supply of exactly MIN_LOCKED_LP is refused: the creator would hold nothing redeemable",
