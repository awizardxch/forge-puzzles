#!/usr/bin/env python3
"""The V13 build is what it claims to be: upstream pinned, curve fresh.

Two kinds of drift this catches, and both would be silent otherwise:

  * an upstream puzzle that is not the community-reviewed one. V13 leans on
...
def main() -> int:
    if not (COMPILED / "manifest.json").is_file():
        print("  [skip] contracts/v13/compiled/manifest.json is absent; run scripts/build-v13.py")
        return 2
    ...
    common = (V13 / "puzzles" / "forge_action_common.rue").read_text(encoding="utf-8")
    ...
    if shutil.which("rue"):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            scratch = pathlib.Path(tmp) / "v13"
            shutil.copytree(V13, scratch, ignore=shutil.ignore_patterns("compiled"))
    ...
    strays = sorted(p.name for p in V13.rglob("*_v[0-9]*.rue"))
    results.append(check("no versioned puzzle names in the V13 set", not strays, ", ".join(strays)))

    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} V13 integrity checks passed")
