...
CAT asset. `forge_v12_create.plan` splits the creation bundle into the creator's
...
import forge_v12_create as create
...
def main() -> int:
    if not (drv.v12_available() and drv.registry_available()):
    ...
    snap = v12.pool_to_snapshot(p.pool)
    check("  the successor pool snapshot rebuilds", v12.snapshot_to_pool(snap).coin == p.pool.coin)
    ...
    for cs in p.router_spends:
        ...
        if bytes(CREATOR_PH) in sol and cs.coin.puzzle_hash == construct_cat_puzzle(CAT_MOD, p.pool.lp_asset_id, v12.OFFER_MOD).get_tree_hash():
    ...
    for label, fn in (
        ...
        except v12.OfferRejected as exc:
