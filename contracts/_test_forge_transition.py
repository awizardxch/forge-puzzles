#!/usr/bin/env python3
"""FORGE transitions built from a real Offer, audited the way a node would.
...
def payouts_in(bundle, asset_id, *extra):
    ...
    # `extra` lets a caller measure a party this module does not know about --
    # the router's own address, for instance. Without it a payout to anyone else
    # reads as zero, which would make a value-conservation check pass or fail for
    # the wrong reason.
    wanted = (USER_PH, TREASURY, bytes32(OFFER_MOD_HASH), *extra)
    return {inner: raw.get(wrap(inner), 0) + raw.get(inner, 0) for inner in wanted}
...
def main() -> int:
    ...
    print(f"FORGE pool created: version={int(pool.config[0])} "
    ...
    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} FORGE transition checks passed")
