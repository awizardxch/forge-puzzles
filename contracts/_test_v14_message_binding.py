#!/usr/bin/env python3
"""Every message V14 sends or receives, and what binds each end of it.
...
`_test_v14_lp_receive_forgery.py`; the reserve messages are
`_test_v14_finalizer.py`; the DAO message is `_test_v14_dao_fee.py`; the registry
and genesis announcements are `_test_v14_registry.py` and `_test_v14_genesis.py`.
...
def lp_message(delta, new_total, root):
    return Program.to(["forge-lp-v14", delta, new_total, root]).get_tree_hash()
...
def main() -> int:
    if not kit.v14_available():
        print("  [skip] the V14 build is absent; run scripts/build-v14.py")
        return 2

    pool = kit.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                         leaves="forge", salt=0x90, last_height=H - 40)
    pool = kit.replace(pool, birth=H - 39)   # V14: birth > last_height
    ...
    # catches it before any announcement is even considered. Keep the coin but name the
    # wrong PARENT and the leaf derives an id no coin in the bundle has -- V14 asserts that
    # derived id is spent (ASSERT_CONCURRENT_SPEND), which is what fires now, before the
    # announcement is even considered. So the puzzle binds WHICH coin was spent and its
    # AMOUNT (spec 1.8); the reserve's recreated amount plus conservation still bind the
    # value flow, and both halves are needed.
    refuses("a swap with no settlement coin at all: caught by conservation, not the announcement",
            lambda: kit.validate(kit.spend_action(
                ...
                [H, 0, 1, 250_000, honest_out, bytes32(b"\x33" * 32), 250_000])[0]),
            "MINTING_COIN")

    coin, settle_spend = kit.offer_settlement_xch(250_000, salt=0x95)
    refuses("a swap that funds the reserve but names the wrong settlement parent: the derived-id spend assert fires",
            lambda: kit.validate(kit.spend_action(
                pool, "forge_action_swap",
                [H, 0, 1, 250_000, honest_out, bytes32(b"\x33" * 32), 250_000],
                extra_spends=[settle_spend])[0]),
            Err.ASSERT_CONCURRENT_SPEND_FAILED.name)
    check("  the honest id is the settlement coin's own name", coin.name() != bytes32(b"\x33" * 32))
    # V14: the same coin, the right parent, but an AMOUNT larger than it holds: the derived
    # id names a coin that does not exist, so the spend assert fires again -- the amount is
    # bound by the puzzle, not only by the ring.
    refuses("a swap naming the right parent but an amount the settlement does not hold is refused",
            lambda: kit.validate(kit.spend_action(
                pool, "forge_action_swap",
                [H, 0, 1, 250_000, honest_out, coin.parent_coin_info, 250_001],
                extra_spends=[settle_spend])[0]),
            Err.ASSERT_CONCURRENT_SPEND_FAILED.name)
    try:
        kit.validate(kit.spend_action(pool, "forge_action_swap",
                                      [H, 0, 1, 250_000, honest_out, *kit.settlement_ref(coin)],
                                      extra_spends=[settle_spend])[0])
        check("  and the honest (parent, amount) pair is accepted", True)
    except (kit.Rejected, ValueError, TypeError) as exc:
        check("  and the honest (parent, amount) pair is accepted", False, code_of(exc))

    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V14 message-binding checks")
