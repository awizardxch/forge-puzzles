...
def main() -> int:
    ...
    refuses("negative tag index fails the spend",
            lambda: kit.validate(kit.spend_pool(pool, new_state, tagged + [(-1, [kit.CREATE_COIN, PAYOUT_PH, 1])],
                                                extra_cats=grow)))
    refuses("a missing parent id fails",
            lambda: kit.validate(kit.spend_pool(pool, new_state, tagged, parent_ids=parents[:1], extra_cats=grow)))
    refuses("an extra parent id fails",
            lambda: kit.validate(kit.spend_pool(pool, new_state, tagged, parent_ids=parents + [parents[0]], extra_cats=grow)))
    refuses("a wrong parent id leaves the reserve's message unmatched",
            lambda: kit.validate(kit.spend_pool(pool, new_state, tagged, parent_ids=[bytes32(b"\x99" * 32), parents[1]],
                                                extra_cats=grow)))
    refuses("swapped parent ids leave both messages unmatched",
            lambda: kit.validate(kit.spend_pool(pool, new_state, tagged, parent_ids=list(reversed(parents)), extra_cats=grow)))
