#!/usr/bin/env python3
"""V14 oracle: every block is credited once, at the price that was in force.
...
V14 keeps the spot it last credited in state (`last_spot`) and credits two intervals:
...
LYING birth is refused is consensus's job: `_test_v14_consensus_timelocks.py` runs
...
def main():
    ...
    bundle, new_state = kit.spend_action(pool, "forge_action_swap",
                                         [H - 60, 0, 1, 250_000, honest, *kit.settlement_ref(settle)], extra_spends=[settle_spend])
    kit.validate(bundle)
    ...
    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V14 oracle checks")
