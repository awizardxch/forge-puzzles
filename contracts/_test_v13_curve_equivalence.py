...
pools and 12 launch-matrix steps ran on has to arrive in V13 unchanged, or the
...
port to that:
  ...
  1. THE CORPUS. compiled/v10-spend-corpus.json holds every V10 pool spend that
     ...
     to the compiled V13 functions, which must accept exactly the recorded
  ...
  2. THE V10 FUNCTIONS THEMSELVES. scripts/build-v13.py compiles the curve
...
COMPILED = CONTRACTS / "v13" / "compiled"
...
class Suite:
    ...
    def same(self, fn, cur_args, v10_args, label):
        ...
        self.expect(f"{label}: V13 {a} vs V10 {b}", a == b)
        return a
...
def main() -> int:
    ...
    print(f"randomized: {cases} pools driven through both the V10 and the V13 functions")
    print()
