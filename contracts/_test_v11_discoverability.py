...
deployment record (.awizard/v11-testnet.json) and asks the node the questions a
...
RECORD = pathlib.Path(__file__).resolve().parent.parent / ".awizard" / "v11-testnet.json"
...
def main() -> int:
    if not RECORD.is_file():
        print("  [skip] no deployment record; run scripts/deploy-v11-testnet.py first")
        return 2
    ...
    for pool in state["pools"]:
        ...
        check("  every generation the finalizer re-created is hinted with the pool launcher", len(singles) == spends, f"{len(singles)} for {spends} spends")
        ...
        check("  LP found by hint plus LP held as other pools' reserves equals total_lp",
              held + in_reserves == pool["state"][1], f"{held} + {in_reserves} vs {pool['state'][1]}")
