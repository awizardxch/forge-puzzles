#!/usr/bin/env python3
"""The oracle finding, both halves, without a chain: V11 accepts the backfill, V12 cannot.

`forge_v12_driver.validate` runs `chia_rs.get_conditions_from_spendbundle`, which
...
So this file closes the gap the testnet halt left open:
  ...
  V12  the same bundle cannot be validated in the first place. A V12 pool spend
       ...
       a V12 pool coin can never be spent in the bundle that created it, and the
       two-generation shape is gone rather than merely detected.

  V12  a single generation lying about its birth to stretch elapsed time is
...
def main() -> int:
    ...
    import _v12_testkit as v12
    if not (v11.v11_available() and v12.v12_available()):
        print("  [skip] a build is absent; run scripts/build-v11.py and scripts/build-v12.py")
        return 2
    ...
    print()
    print("V12, the same bundle, whatever birth the successor claims:")
    # Two different lines refuse it, and which one is the interesting part. A birth the
    # successor could plausibly claim trips consensus's ban on a birth condition for a
    # coin created in the same bundle. Its TRUE birth -- the block it is created in --
    # never even reaches that rule, because the prologue's own `h >= birth` cannot hold
    # for a height the bundle is also required to be mined at or before.
    EPHEMERAL, PUZZLE = "ephemeral", "puzzle"
    for label, claimed, expected in (
            ("backfilling (claims it was born 31 blocks ago)", H - 31, EPHEMERAL),
            ("claiming generation 1's own birth", born, EPHEMERAL),
            ("truthful (claims the block it is created in)", PEAK + 1, PUZZLE)):
        try:
            agg12, births12, _, _ = two_generation(v12, born, claimed, versioned_birth=True)
            err12, _ = timelocks(v12, agg12, births12)
            check(f"{label}: refused", False, f"ACCEPTED with {err12}")
            continue
        except (v12.Rejected, ValueError, TypeError) as exc:
            text = str(exc)
        if expected is EPHEMERAL:
            ...
            check(f"{label}: the prologue's own `h >= birth` refuses it first", got, text[:70])

    print()
    print("V12, one generation, lying about its birth:")
    pool = v12.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                         leaves="forge", salt=0x40, last_height=H - 40)
    honest, _ = v12.spend_action(v12.replace(pool, birth=born), "forge_action_observe", [H])
    check("every V12 pool spend asserts a birth height", birth_asserts(v12, honest) >= 1)
    err, _ = timelocks(v12, honest, {cs.coin.name(): born for cs in honest.coin_spends})
    ...
    for label, claimed in (("understating birth by 31 blocks (stretches elapsed)", born - 31),
        ...
        bundle, _ = v12.spend_action(v12.replace(pool, birth=claimed), "forge_action_observe", [H])
        err, _ = timelocks(v12, bundle, {cs.coin.name(): born for cs in bundle.coin_spends})
    ...
    print()
    print("V12, the claimed height is boxed in from both sides:")
    future, _ = v12.spend_action(v12.replace(pool, birth=born), "forge_action_observe", [PEAK + 1])
    err, _ = timelocks(v12, future, {cs.coin.name(): born for cs in future.coin_spends})
    ...
    stale_pool = v12.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                               leaves="forge", salt=0x41, last_height=H - 80)
    stale, _ = v12.spend_action(v12.replace(stale_pool, birth=H - 60), "forge_action_observe", [H - 40])
    err, _ = timelocks(v12, stale, {cs.coin.name(): H - 60 for cs in stale.coin_spends})
    ...
    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V12 consensus time-lock checks")
