...
def registration(reg, pool, left, right, slots, fee=None, kv=None, launcher_pool=None, with_fee=True,
                 with_launcher=True, genesis_mint=None, genesis_cat_parent=False, burn=None, with_reserves=True):
    ...
    if with_launcher:
        ...
        extra.append(spend)
    if with_reserves:
        # V14: the reserve launchers, whose announcements `register` asserts from the parents it derives
        extra.extend(kit.reserve_launcher_spends(launcher_pool or pool))
        extra.extend(kit.lp_genesis_mint_spends(launcher_pool or pool, RECIPIENT, mint=genesis_mint,
                                                with_cat_parent=genesis_cat_parent, burn=burn))
    if with_fee:
...
def main() -> int:
    ...
    print("register:")
    # V14 (CHIP-0062 review P1, "the final LP position cannot redeem"): the locked minimum is
    # set at genesis, so a supply below it can never be registered.
    low = kit.make_pool([None, CATS[0]], [10_000_000, 20_000_000], total_lp=kit.LOCKED_BURN - 1, leaves="forge", salt=0x5F)
    refuses("V14: a genesis supply below LOCKED_BURN is refused at registration",
            lambda: kit.validate(registration(reg1, low, (kit.MIN_KEY, kit.ZERO_32, kit.MIN_KEY),
    ...
    if out:
        ...
        burn_ph = kit.construct_cat_puzzle(kit.CAT_MOD, pool_a.lp_asset_id, kit.Program.to(kit.ZERO_32)).get_tree_hash_precalc(kit.ZERO_32)
        # V14: the settlement burns the floor and pays the creator the rest; `register` asserted the burn
        check("  the genesis LP above the locked floor landed at the creator, hinted, authorized by the launcher",
              any(p == lp_ph and a == pool_a.state[1] - kit.LOCKED_BURN and h == bytes(RECIPIENT) for p, a, h in hinted))
        check("  and LOCKED_BURN of it was burned to the zero puzzle hash in the same bundle",
              any(p == burn_ph and a == kit.LOCKED_BURN for p, a, _ in hinted))
    ...
    # ---- V14: what the second audit found free ----
    print("V14 admission (second audit):")
    refuses("a genesis that burns nothing (the whole supply to the creator, V12's website lane) is refused",
            lambda: kit.validate(registration(reg2, pool_b, left_b, right_b, slots, burn=0)[0]))
    refuses("a genesis that burns one unit short of the floor is refused",
            lambda: kit.validate(registration(reg2, pool_b, left_b, right_b, slots, burn=kit.LOCKED_BURN - 1)[0]))
    boundary = kit.make_pool([None, CATS[4]], [1_000_000, 1_000_000], total_lp=kit.LOCKED_BURN, leaves="forge", salt=0x56)
    refuses("a genesis supply of exactly LOCKED_BURN is refused: the creator would hold nothing redeemable",
            lambda: kit.validate(registration(reg2, boundary, left_b, right_b, slots)[0]))
    squat = kit.make_pool([None, CATS[4]], [1_000_000, 1_000_000], total_lp=1_000_000, leaves="forge", salt=0x57,
                          protocol_ph=bytes32(b"f" * 32))
    refuses("a pool whose protocol fee recipient is not the registry's is refused (the fee-recipient squat)",
            lambda: kit.validate(registration(reg2, squat, left_b, right_b, slots)[0]))
    scaled = kit.make_registry(salt=0x22, price_scale=kit.PRICE_SCALE * 2).advance([1, 0])
    refuses("a registry pinned to another price scale refuses a pool at the driver's",
            lambda: kit.validate(registration(scaled, pool_b, left_b, right_b, slots)[0]))
    windowed = kit.make_registry(salt=0x22, oracle_window=kit.ORACLE_WINDOW + 1).advance([1, 0])
    refuses("...and one pinned to another oracle window refuses it too",
            lambda: kit.validate(registration(windowed, pool_b, left_b, right_b, slots)[0]))
    # V14 (mutation rule): the tests above hand `squat`, `boundary` and the pinned-parameter pools
    # pool_b's neighbours, so the key bracket refuses them before `valid_pool` is consulted --
    # and the mutation run reported `assert valid_pool(...)` UNREACHED. These bracket each pool
    # by its OWN neighbours, so the only line that can refuse is valid_pool.
    print("V14: valid_pool pinned at the leaf, each pool bracketed by its own neighbours:")

    def own_bracket(pool):
        k = kit.pool_key(pool.config())
        if k < key_a:
            return (kit.MIN_KEY, kit.ZERO_32, kit.MIN_KEY), (key_a, pool_a.launcher_id, kit.MAX_KEY)
        return (key_a, pool_a.launcher_id, kit.MIN_KEY), (kit.MAX_KEY, kit.ZERO_32, kit.MAX_KEY)

    good = kit.make_pool([None, CATS[7]], [1_000_000, 1_000_000], total_lp=1_000_000, leaves="forge", salt=0x5A)
    accepts("  a well-formed pool, correctly bracketed, is accepted (control)",
            lambda: kit.validate(registration(reg2, good, *own_bracket(good), slots)[0]))
    for label, bad in (
        ("a protocol fee recipient that is not the registry's",
         kit.make_pool([None, CATS[5]], [1_000_000, 1_000_000], total_lp=1_000_000, leaves="forge", salt=0x58, protocol_ph=bytes32(b"f" * 32))),
        ("a genesis supply of exactly LOCKED_BURN",
         kit.make_pool([None, CATS[6]], [1_000_000, 1_000_000], total_lp=kit.LOCKED_BURN, leaves="forge", salt=0x59)),
        ("a DAO rate with no DAO recipient",
         kit.make_pool([None, CATS[8]], [1_000_000, 1_000_000], total_lp=1_000_000, leaves="forge", salt=0x5B, dao_fee_bps=10)),
    ):
        l, r = own_bracket(bad)
        refuses(f"  {label}, correctly bracketed, is refused by valid_pool alone",
                lambda: kit.validate(registration(reg2, bad, l, r, slots)[0]))
