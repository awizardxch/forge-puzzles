...
def snapshot_roundtrip(pool):
    snap = v13.pool_to_snapshot(pool)
    back = v13.snapshot_to_pool(json.loads(json.dumps(snap)))
    return snap, back
...
def swap_case(label, pool, i_in, i_out, gross, ask_bps=9_900, expect_ok=True):
    ...
    honest = v13.forge_math.swap_output(r[i_in], r[i_out], gross, pool.fee_bps, w[i_in], w[i_out])
    ...
    check("  successor snapshot is V11 and rebuilds to the successor pool",
          out["pool"]["protocol_version"] == drv.PROTOCOL_VERSION and v13.snapshot_to_pool(out["pool"]).coin.name().hex() == out["forge"]["successor_coin_id"])
...
def refund_at_real_rate(label, pool, i_in, i_out, gross, salt):
    ...
    r, w = pool.state[0], pool.weights
    honest = v13.forge_math.swap_output(r[i_in], r[i_out], net, pool.fee_bps, w[i_in], w[i_out])
    ...
    out = run({"action": "swap", "offer": offer.to_bech32(), "pool": v13.pool_to_snapshot(pool),
...
def add_case(label, pool, pct, extra_xch=0, expect_ok=True):
    ...
    honest = v13.forge_math.invariant_lp_mint(pool.state[0], deposits, pool.state[1], pool.fee_bps, pool.weights, version=10)
    offered = {}
    ...
    check("  successor snapshot rebuilds", v13.snapshot_to_pool(out["pool"]).coin.name().hex() == out["forge"]["successor_coin_id"])
    return out
...
def remove_case(label, pool, burn, ask_bps=9_900, expect_ok=True):
    ...
    snap, _ = snapshot_roundtrip(pool)
    vf = v13.forge_math.vault_fee_bps(len(pool.state[0]), 10, pool.fee_bps)
    payouts = v13.forge_math.withdrawal_amounts(pool.state[0], burn, pool.state[1], vf)
...
def main() -> int:
    ...
    bad = dict(v13.pool_to_snapshot(pair)); bad["state"] = dict(bad["state"]); bad["state"]["total_lp"] = "999"
    try:
        v13.snapshot_to_pool(bad); check("  a snapshot with altered state is refused", False)
    except ValueError as exc:
        check("  a snapshot with altered state is refused", "does not hash" in str(exc))
    try:
        forge_stdin._pool(v13.pool_to_snapshot(pair)); check("  the V3Pool reader refuses a V11 snapshot", False)
    except ValueError as exc:
        check("  the V3Pool reader refuses a V13 snapshot", "versioned offer lane" in str(exc))
    ...
    print("lane guards:")
    snap = v13.pool_to_snapshot(pair)
    ...
    print()
    passed = sum(results)
    print(f"{passed}/{len(results)} V13 offer-lane checks passed")
