...
from forge_v12_driver import replace  # noqa: E402
from forge_v12_offer import pool_to_snapshot, snapshot_to_pool  # noqa: E402
...
def _leaf_names(pool: drv.V12Pool) -> dict[bytes, str]:
...
def replay_spend(pool: drv.V12Pool, solution: Program) -> tuple[drv.V12Pool, list[str]]:
    ...
    for leaf_prog, leaf_sol in zip(puzzles.as_iter(), solutions.as_iter()):
        ...
        names.append(name)
        # V12: an on-chain leaf solution is `[h, birth, ...]`, and `run_leaf` inserts
...
def resync(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # V12: take the tip's birth height from the CHAIN, not from the replay.
