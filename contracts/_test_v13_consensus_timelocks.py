#!/usr/bin/env python3
"""The oracle finding, both halves, without a chain: V11 accepts the backfill, V13 cannot.

`forge_v13_driver.validate` runs `chia_rs.get_conditions_from_spendbundle`, which
...
So this file closes the gap the testnet halt left open:
  ...
  V13  the same bundle cannot be validated in the first place. A V13 pool spend
       ...
       a V13 pool coin can never be spent in the bundle that created it, and the
       two-generation shape is gone rather than merely detected.

  V13  a single generation lying about its birth to stretch elapsed time is
...
def main() -> int:
    ...
    import _v13_testkit as v13
    if not (v11.v11_available() and v13.v13_available()):
        print("  [skip] a build is absent; run scripts/build-v13.py and scripts/build-v13.py")
        return 2

    born = H - 39   # V13: a coin is born AFTER the height its state last claimed (H - 40)
    ...
    print()
    print("V13, the same bundle, whatever birth the successor claims:")
    # Two different lines refuse it, and which one is the interesting part. A birth at or
    # below the height generation 1 claimed is refused by the prologue itself (V13 asserts
    ...
    for label, claimed, expected in (
        ...
        try:
            agg12, births12, _, _ = two_generation(v13, born, claimed, versioned_birth=True)
            err12, _ = timelocks(v13, agg12, births12)
            check(f"{label}: refused", False, f"ACCEPTED with {err12}")
            continue
        except (v13.Rejected, ValueError, TypeError) as exc:
            text = str(exc)
    ...
    print()
    print("V13, one generation, lying about its birth:")
    ...
    pool = v13.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                         leaves="forge", salt=0x40, last_height=H - 80)
    honest, _ = v13.spend_action(v13.replace(pool, birth=born), "forge_action_observe", [H])
    check("every V13 pool spend asserts a birth height", birth_asserts(v13, honest) >= 1)
    err, _ = timelocks(v13, honest, {cs.coin.name(): born for cs in honest.coin_spends})
    check("the honest spend passes the time-lock rule", err is None, "" if err is None else str(err))

    try:
        v13.spend_action(v13.replace(pool, birth=H - 80), "forge_action_observe", [H])
    ...
    for label, claimed in (("understating birth by 31 blocks (stretches elapsed)", born - 31),
        ...
        bundle, _ = v13.spend_action(v13.replace(pool, birth=claimed), "forge_action_observe", [H])
        err, _ = timelocks(v13, bundle, {cs.coin.name(): born for cs in bundle.coin_spends})
    ...
    print()
    print("V13, the claimed height is boxed in from both sides:")
    future, _ = v13.spend_action(v13.replace(pool, birth=born), "forge_action_observe", [PEAK + 1])
    err, _ = timelocks(v13, future, {cs.coin.name(): born for cs in future.coin_spends})
    ...
    stale_pool = v13.make_pool([None, CAT], [10_000_000, 20_000_000], total_lp=5_000_000,
                               leaves="forge", salt=0x41, last_height=H - 80)
    stale, _ = v13.spend_action(v13.replace(stale_pool, birth=H - 60), "forge_action_observe", [H - 40])
    err, _ = timelocks(v13, stale, {cs.coin.name(): H - 60 for cs in stale.coin_spends})
    ...
    print()
    print(f"{'ALL PASSED' if not FAILED else f'{FAILED} FAILED'} -- V13 consensus time-lock checks")
