...
Delete that TAIL assert and rebuild (scripts/mutate-v12.py) and B flips to
...
def lp_message(lp_delta: int, new_total_lp: int, next_state_root: bytes32) -> bytes32:
    """The rue mirror: tree_hash(["forge-lp-v12", lp_delta, new_total_lp, next_state_root])."""
    return Program.to(["forge-lp-v12", lp_delta, new_total_lp, next_state_root]).get_tree_hash()
...
def main():
    ...
    # H1/H2 pin `exact_withdrawal`, the multi-line assert scripts/mutate-v12.py cannot
