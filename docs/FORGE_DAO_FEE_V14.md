# DAO fee — V14, as built, with the threat model checked off
...
**V14 note (2026-09-16).** The DAO fee is unchanged between V14 and V14: the `dao_fee`
leaf, the rate in state, the DAO slice in `swap` and the merged `collect` payout are
byte-for-byte the same logic under `contracts/v14`, re-verified by `_test_v14_dao_fee.py`
(37/37) and the mutation run. What V14 changes around it -- the reserve launcher, the
settlement binding, `LOCKED_BURN = 1` -- touches no DAO path. "The second external
review" below refers to the six-agent re-audit of V12 posted to Chia-Network/chips#217
on 2026-09-14; the fourth (trgarrett) and fifth reviews of 2026-09-15 raised nothing
against the DAO fee.
...
shipped, read against `contracts/v14`. Each vector says what closes it in the
...
The short version: the feature is live on testnet11, six of the thirty-two V14
...
The second change is V14's, from the second external review: **`collect` pays one
...
V14 adds a second, unrelated guarantee in the same area. The finalizer now asserts
...
Pinned: `_test_v14_dao_fee.py`; `_test_v14_finalizer.py`;
`_test_v12_v14_review_findings.py` (the cross-leaf drain, accepted against V12 and
refused against V14).
...
Pinned: `_test_v14_dao_fee.py` checks every reserve is re-created unchanged.
...
subtracts two, and `v14_ops.swap_bundle` subtracted only the protocol fee, so it
...
### V8 — Coincident recipients — **new in V14, closed**
...
V14 pays one coin of `fee + dao` when the recipients match:
...
Pinned: `_test_v14_actions.py` checks exactly one coin of the summed amount, and
`_test_v12_v14_review_findings.py` reproduces the V12 `DUPLICATE_OUTPUT` beside
V14 paying the merged coin.
...
| Payment, merged when recipients match | `forge_action_collect.rue` | live, V14 |
...
| Vectors V1 to V8 | `_test_v14_dao_fee.py` (37 checks) | green |
