#!/usr/bin/env python3
"""Build the Forge V13 rue project and check it against the upstream pins.

Three things come out of contracts/v13/compiled/:

  * every vendored upstream puzzle, recompiled from its verbatim source and
    required to hash to the value pinned in contracts/v13/pins.json;
  ...
  * the same functions cut out of the V10 pool puzzle, compiled with the V10
    bodies untouched, as the reference the V13 module is compared against.
...
def main() -> int:
    ...
    pins = json.loads((V13 / "pins.json").read_text(encoding="utf-8"))
    ...
    out = rue("build", "--hex", "--hash", "--all", ".", cwd=V13)
    if out.returncode != 0:
    ...
    for hex_path in sorted(COMPILED.glob("*.rue.hex")):
        ...
        manifest["outputs"][hex_path.name] = {
            "tree_hash": built, "source": src.relative_to(V13).as_posix(),
    ...
    #     inner is V13's clsp (hint added), compiled here with clvm_tools.
    print("LP inners:")
    ...
    mint_src = V13 / "clsp" / "forge_lp_mint_inner.clsp"
    ...
    ok &= good
    print(f"  [{'PASS' if good else 'FAIL'}] {'forge_lp_mint_inner (V13)':28s} {mint_hash}")
    ...
    # 1d. The merkle root over exactly the six Forge leaves, in driver order (V13.1 added dao_fee).
