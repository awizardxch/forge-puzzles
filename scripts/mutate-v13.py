#!/usr/bin/env python3
"""Mutation testing for the V13 leaves: which assertions are actually load-bearing?
...
redundant, or untested? Both answers are fine. Not knowing which is not.
    ...
    python scripts/mutate-v13.py                      # every Forge leaf
    python scripts/mutate-v13.py --leaf forge_action_swap
    python scripts/mutate-v13.py --suite _test_v13_manipulation.py

Needs `rue` on PATH. Nothing is ever written into contracts/v13/compiled: each
...
# --project picks the rue project. V13 is the shipping revision and the default;
...
DEFAULT_SUITES = [
    "_test_v13_actions.py",
    "_test_v13_dao_fee.py",
    "_test_v13_registry.py",
    ...
    "_test_v13_lp_receive_forgery.py",
]
...
def main() -> int:
    ...
    parser.add_argument("--project", choices=("v13", "v11"), default="v13", help="which rue project to mutate")
    ...
    PUZZLES = V11 / "puzzles"
    DEFAULT_SUITES = [x.replace("_v13_", f"_{PROJECT}_") for x in DEFAULT_SUITES]
    if PROJECT == "v13":
        DEFAULT_SUITES += ["_test_v13_genesis.py", "_test_v13_oracle.py", "_test_v13_finalizer.py"]
