...
> **Status: testnet research. Unaudited.** V11.1 is live on testnet11 with 20
> pools. No revision has been reviewed by anyone outside the team that wrote it.
> Every leaf carries this warning in its own header, and it is not decoration.
...
```
contracts/
  v11/                 the shipping pool, a rue project
...
**Start at [`docs/FORGE_PUZZLE_V11.md`](docs/FORGE_PUZZLE_V11.md)**: coin layout,
config and state, the prologue, the five leaves, the finalizer, authorization,
fees, the registry.
...
verbatim and required to hash to the values in `contracts/v11/pins.json`; Forge
...
```bash
python scripts/build-v11.py     # in the monorepo; rebuilds and re-checks the pins
rue build --hex --hash --all .  # in contracts/v11, to build in place
```
...
```bash
python contracts/_test_v11_actions.py        # the five leaves, accept and refuse
python contracts/_test_v11_manipulation.py   # flash-loan-analog sequences
python contracts/_test_v11_curve_equivalence.py
...
- [`docs/FORGE_V11_CLVM_PASS.md`](docs/FORGE_V11_CLVM_PASS.md) — the written
  CLVM pass over every leaf: what each asserts, what it emits, which test pins
  each refusal.
